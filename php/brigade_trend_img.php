...
try
{
	...
	foreach ($tmp_array as $key=>$record) {
		...
		if (($arg_brig==$brigade or $arg_brig==0) and ($arg_furnace==$record['ID_Channel']-25 or $arg_furnace==0)) {
			...
			if (!isset($data[$record['ID_Channel']-26])) $data[$record['ID_Channel']-26]=array();
			if (!isset($data[$record['ID_Channel']-26][$brigade])) $data[$record['ID_Channel']-26][$brigade]=array();
			if (!isset($data[$record['ID_Channel']-26][$brigade][$dt->format('d.m.Y')])) $data[$record['ID_Channel']-26][$brigade][$dt->format('d.m.Y')]=array('arkalyk'=>0,'berlinka'=>0,'gase'=>0,'elec'=>0,'ud_gase'=>0,'ud_elec'=>0,'water'=>0,'density'=>0);
			$data[$record['ID_Channel']-26][$brigade][$dt->format('d.m.Y')]['gase']+=$record['Value']*1000;
		}
	}
	...
	$graph->yaxis->SetTitle("Удельный расход коксового газа, м3/т",'middle');
	...
	foreach ($result_array as $key=>$record) {
			...
			foreach ($record['data'] as $key2 => $record2) {
				...
				array_push($values,$record2[5]>0.0?$record2[5]:0);
			}
			//print_r($time);
			//print_r($values);
