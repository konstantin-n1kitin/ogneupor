...
//------------------------------------------------------------------------------
//Пар ЦСИ - технология суточный отчёт
  public function csi_steam_teh_daily_report($date)
  {
//пар
//2245	39	Fисх.пар
//2276	39	Зашкал dРпара
...
//2568	39	Тепловая энергия
    //задаем константы IDшек
    $IDs = array('T_s'=>    '2515',   //температура прямой воды
                 ...
                 'T_r'=>    '2517',   //температура обратной воды
...
//------------------------------------------------------------------------------
//коксовый газ на сушильные барабаны ДПУ суточный отчёт
  public function csi_rotary_driers_coke_gas_DPU_daily_report($date)
  {
//коксовый газ
    ...
    foreach($data as $str_num => $rec)
    {
      if ($str_num%2!=0)
      {
        ...
        $tmp_rec = array(date_format($rec_date,'H:i:s'),
                         ...
                         round((($data[$str_num]['P'])+($data[$str_num-1]['P']))/2,2),
                         round(($data[$str_num]['P_br']+$data[$str_num-1]['P_br'])/3600,1),
                         round((($data[$str_num]['dP'])+($data[$str_num-1]['dP']))/2,4),
                         round(($data[$str_num]['dP_br']+$data[$str_num-1]['dP_br'])/3600,1),
                         round((($data[$str_num]['V'])+($data[$str_num-1]['V'])),2));
        $total+=round((($data[$str_num]['V'])+($data[$str_num-1]['V'])),2);
        array_push($tmp_array, $tmp_rec);
...
//------------------------------------------------------------------------------
//коксовый газ на сушильные барабаны ДП и ФУ суточный отчёт
  public function csi_rotary_driers_coke_gas_DPandFU_daily_report($date)
  {
//коксовый газ
...
//------------------------------------------------------------------------------
//коксовый газ на нагревательные печи 1 и 2 термического участка ЦСИ суточный отчёт
  public function csi_coke_gas_heating_fu_1_2_daily_report($date)
  {
    //задаем константы IDшек
...
//    print_r($data);
    foreach($data as $str_num => $rec)
    {
      if ($str_num%2!=0)
      {
		...
		$T = round((($data[$str_num]['T'])+($data[$str_num-1]['T']))/2,2);
		if ($T == -0) $T = 0;
		$dP = round((($data[$str_num]['dP'])+($data[$str_num-1]['dP']))/2,2);
		if ($dP == -0) $dP = 0;
		$P = round((($data[$str_num]['P'])+($data[$str_num-1]['P']))/2,2);
		if ($P == -0) $P = 0;
		$V = round((($data[$str_num]['V'])+($data[$str_num-1]['V'])),2);
		if ($V == -0) $V = 0;
