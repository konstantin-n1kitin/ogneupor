...
class Model_Cshibrigadereports extends Kohana_Model
{
//------------------------------------------------------------------------------
//коксовый газ на вращающуюся печь №1
  public function cshi_rotating_oven_1_coke_gas_brigade_report($arg_dt_begin, $arg_dt_end, $arg_brig)
  {
//коксовый газ
...
//3147	3	обрыв dР В/П1
    ...
    $IDs = array('V'=>'26');
		$Brig=array(array(1,3),array(4,1),array(2,4),array(3,2));
    ...
    $date_begin->add(new DateInterval('PT8H00M'));
    $date_end->add(new DateInterval('PT32H00M'));
   	$dbhost = "ASKUSERVER2";
		$dbname = "oup";
		$dbuser = "sa";
		$dbpass = "metallurg";
    ...
    $interval_end->add(new DateInterval('PT12H00M'));
    $input_array_ind = 0;
    $output_array_ind = 0;
    $total = 0;
    $rec_date = $interval_begin;
		$shift=1;
    $tmp_rec_array = array('DT'=>'','Shift'=>'','Brig'=>'','V'=>'');
    ...
    while (isset($data[$input_array_ind])) //пробегаемся по массиву значений
    {
      ...
      else
      {
       //вычисляем средние значения там где нужно
        $tmp_rec_array['DT'] = $interval_begin->format('d.m.Y');
				$tmp_rec_array['Brig']=$Brig[((($interval_begin->format('Y')-2016)*365+(integer)(($interval_begin->format('Y')-2016)/4)+1)+$month[$interval_begin->format('m')-1]+$interval_begin->format('d')+(($interval_begin->format('m')>2 & (($interval_begin->format('Y')) % 4)==0) ? 1 : 0)-1)%4][$shift-1];
				$tmp_rec_array['Shift'] = $shift++;
				if ($shift>2) $shift=1;
        if ($arg_brig==0 || $arg_brig==$tmp_rec_array['Brig']) {
        ...
        $interval_begin = new DateTime($interval_end->format('d.m.Y H:i:s'));
        $interval_end->add(new DateInterval('PT12H00M'));
        $output_array_ind++;
      }
    }
    $result_array['column_titles']=array('Число','Смена','Бригада','Объем, м<sup>3</sup> (V)');
...
//------------------------------------------------------------------------------
//коксовый газ на вращающуюся печь №1
  public function cshi_rotating_oven_2_coke_gas_brigade_report($arg_dt_begin, $arg_dt_end, $arg_brig)
  {
//коксовый газ
//20	3	Fкок.газ исх. В/П1
//26	3	Vкок.газ В/П1
...
//------------------------------------------------------------------------------
//коксовый газ на вращающуюся печь №1
  public function cshi_compressed_air_gas_cleaning_brigade_report($arg_dt_begin, $arg_dt_end, $arg_brig)
  {
//коксовый газ
//20	3	Fкок.газ исх. В/П1
//26	3	Vкок.газ В/П1
...
//3147	3	обрыв dР В/П1
  ...
  //------------------------------------------------------------------------------
//Индустрия 4.0 Бригадный отчет ВП
  public function cshi_rotary_furnace_brigade_report($arg_dt_begin, $arg_dt_end, $arg_brig, $arg_furnace)
  {
set_time_limit(600);
  $data = array();
    $date_begin = new DateTime($arg_dt_begin);
		...
		foreach($record as $key2=>$record2) {
			if ($key2==0) {
				...
				foreach ($furnace[$key] as $key3=>$record3) { //Цикл 3
					...
					/*else if (strtotime($record3["dt_begin"])<=$interval_begin->getTimestamp() and strtotime($record3["dt_end"])<=$interval_end->getTimestamp()) {
						array_push($furnace[$key],array("dt_begin"=>$record3["dt_begin"],"dt_end"=>$interval_begin->format('Y-m-d H:i:s'),"furnace"=>$record3["furnace"],
														 "brigade"=>$record3["brigade"],"conveyer"=>$record3["conveyer"],"glina"=>$interval_value));
						array_push($furnace[$key],array("dt_begin"=>$interval_begin->format('Y-m-d H:i:s'),"dt_end"=>$record3["dt_end"],"furnace"=>$record3["furnace"],
														 "brigade"=>$record3["brigade"],"conveyer"=>$record3["conveyer"],"glina"=>""));
						unset($furnace[$key][$key3]);
					}*/
				}
	...
	foreach ($furnace as $key=>$record) {
      ...
      //print_r($record2);
			if (($arg_brig==$record2["brigade"] or $arg_brig==0) and ($arg_furnace==($record2["furnace"]+1) or $arg_furnace==0)) {
				...
				//Считаем данные
				//print_r("Z1=".$z1.";Z2=".$z2."; ");
				$data[$record2["furnace"]][$record2["brigade"]][$date_begin->format('d.m.Y')][($record2["glina"]==0)?'arkalyk':'berlinka']+=($z2-$z1)/1000;
			}
		}
	}
	//Добавляем информацию по расходу
	$dbhost = "ASKUSERVER2";$dbname = "oup";$dbuser = "sa";$dbpass = "metallurg";
    $db = new PDO ('odbc:DRIVER={SQL Server};SERVER='.$dbhost.';database='.$dbname.';Uid='.$dbuser.';Pwd='.$dbpass.';');
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
	$sql = "SELECT ID_Channel, MeasureDate, Value
            FROM Mains
            WHERE (ID_Channel = 26 or ID_Channel = 27) AND (MeasureDate > :dt_begin) AND (MeasureDate <= :dt_end)
            ORDER BY MeasureDate;";
	$date_begin = new DateTime($arg_dt_begin);
    $date_end = new DateTime($arg_dt_end);
    $date_begin->sub(new DateInterval('PT4H00M'));
	...
	foreach ($tmp_array as $key=>$record) {
		...
		if (($arg_brig==$brigade or $arg_brig==0) and ($arg_furnace==$record['ID_Channel']-25 or $arg_furnace==0)) {
			$dt->add(new DateInterval('PT4H00M'));
			$data[$record['ID_Channel']-26][$brigade][$dt->format('d.m.Y')]['gase']+=$record['Value'];
		}
	}
	...
	foreach ($tmp_array as $key=>$record) {
		...
		if (($arg_brig==$brigade or $arg_brig==0) and ($arg_furnace==$furnace_num or $arg_furnace==0)) {
			$dt->add(new DateInterval('PT4H00M'));
			$data[$furnace_num-1][$brigade][$dt->format('d.m.Y')]['elec']+=$record['Value'];
		}
	}
	//Добавляем информацию по водопоглощению и кажущейся плотности
	$dbhost = "TPL-server";$dbname = "tpl";$dbuser = "sa";$dbpass = "tpl";
    $db = new PDO ('odbc:DRIVER={SQL Server};SERVER='.$dbhost.';database='.$dbname.';Uid='.$dbuser.';Pwd='.$dbpass.';');
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
	$sql = "SELECT dt, shift, water, density
            FROM teh_params
	...
	foreach ($tmp_array as $key=>$record) {
		...
		$brigade=$Brig[((($dt->format('Y')-2016)*365+(integer)(($dt->format('Y')-2016)/4)+1)+$month[$dt->format('m')-1]+$dt->format('d')+(($dt->format('m')>2 & (($dt->format('Y')) % 4)==0) ? 1 : 0)-1)%4][$record['shift']-1];
		foreach ($data as $furnace_number=>$record2) {
			if (isset($record2[$brigade][$dt->format('d.m.Y')])) {
				$data[$furnace_number][$brigade][$dt->format('d.m.Y')]['water']=$record['water'];
				$data[$furnace_number][$brigade][$dt->format('d.m.Y')]['density']=$record['density'];
			}
		}
	}
	...
	//Формируем отчет
	$result_array=array();
	$column_titles=array('Дата','Аркалык, т','Берлинка, т','Расход коксового газа за смену, м<sup>3</sup>','Расход электричества за смену, кВт*ч','Удельный расход коксового газа, м<sup>3</sup>/т', 'Удельный расход электричества, кВт*ч/т','Водопоглощение, %','Кажущаяся плотность, г/см<sup>3</sup>');
    foreach ($data as $furnace_number=>$record) {
		foreach ($record as $brigate_number=>$record2) {
			if (($arg_brig==$brigate_number or $arg_brig==0) and ($arg_furnace==$furnace_number+1 or $arg_furnace==0)) {
				...
				foreach ($record2 as $dt=>$record3) {
					array_push($result_array[($furnace_number*10)+$brigate_number]['data'],array($dt,($record3['arkalyk']!='' and $record3['arkalyk']>0)?round($record3['arkalyk'],1):0,($record3['berlinka']!='' and $record3['berlinka']>0)?round($record3['berlinka'],1):0,($record3['gase']!='')?round($record3['gase'],1):0,($record3['elec']!='')?round($record3['elec'],1):0,
																								($record3['ud_gase']!='' and $record3['ud_gase']>0)?round($record3['ud_gase'],2):0,($record3['ud_elec']!='' and $record3['ud_elec']>0)?round($record3['ud_elec'],2):0,($record3['water']!='')?round($record3['water'],1):0,($record3['density']!='')?round($record3['density'],1):0));
				}
			}
		}
	}
    ...
    //------------------------------------------------------------------------------
//Индустрия 4.0 Бригадный отчет ВП
  public function cshi_rotary_furnace_brigade_report2($arg_dt_begin, $arg_dt_end, $arg_brig, $arg_furnace)
  {
set_time_limit(600);
  $data = array();
    $date_begin = new DateTime($arg_dt_begin);
	...
	foreach ($furnace as $key=>$record) {
      ...
      //print_r($record2);
			if (($arg_brig==$record2["brigade"] or $arg_brig==0) and ($arg_furnace==($record2["furnace"]+1) or $arg_furnace==0)) {
				...
				$z2=$tmp_array[0]["Z3"];
				//Считаем данные
				$brigade_weight[$record2["brigade"]]+=($z2-$z1)/1000;
			}
		}
	}
	//print_r($brigade_weight);
	//Добавляем информацию по расходу
	$dbhost = "ASKUSERVER2";$dbname = "oup";$dbuser = "sa";$dbpass = "metallurg";
    $db = new PDO ('odbc:DRIVER={SQL Server};SERVER='.$dbhost.';database='.$dbname.';Uid='.$dbuser.';Pwd='.$dbpass.';');
    $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
	...
	foreach ($tmp_array as $key=>$record) {
		...
		$brigade=$Brig[((($dt->format('Y')-2016)*365+(integer)(($dt->format('Y')-2016)/4)+1)+$month[$dt->format('m')-1]+$dt->format('d')+(($dt->format('m')>2 & (($dt->format('Y')) % 4)==0) ? 1 : 0)-1)%4][$shift];
		if (($arg_brig==$brigade or $arg_brig==0) and ($arg_furnace==$record['ID_Channel']-25 or $arg_furnace==0)) {
			$brigade_gase[$brigade]+=$record['Value'];
		}
	}
	//Добавляем информацию по электроэнергии
	$furnace_1_ids = array(4254,4470,4578,4614,4660,4434);
	...
	foreach ($tmp_array as $key=>$record) {
		...
		$furnace_num = in_array($record['ID_Channel'], $furnace_1_ids) ? 1 : 2;
		if (($arg_brig==$brigade or $arg_brig==0) and ($arg_furnace==$furnace_num or $arg_furnace==0)) {
			$brigade_elec[$brigade]+=$record['Value'];
		}
	}
	
	//Сортируем отчет
	$table1=array(array(),array());
	for ($i=1;$i<=4;$i++) {
		array_push($table1[0],$i);
		array_push($table1[1],($brigade_weight[$i]>0) ? $brigade_gase[$i]/$brigade_weight[$i] : 0);
	}
	array_multisort($table1[1],$table1[0]);
	
	$table2=array(array(),array());
	for ($i=1;$i<=4;$i++) {
		array_push($table2[0],$i);
		array_push($table2[1],($brigade_weight[$i]>0) ? $brigade_elec[$i]/$brigade_weight[$i] : 0);
	}
	array_multisort($table2[1],$table2[0]);
	
	//Формируем отчет
	$result_array=array();
	array_push($result_array, array('caption'=>'Удельный расход коксового газа за весь период','column_titles'=>array('Место',
	'Бригада', 'Удельный расход коксового газа, м<sup>3</sup>/т'),'data'=>array(array(1,$table1[0][0],round($table1[1][0],2)),array(2,$table1[0][1],round($table1[1][1],2)),array(3,$table1[0][2],round($table1[1][2],2)),array(4,$table1[0][3],round($table1[1][3],2)))));
	array_push($result_array, array('caption'=>'Удельный расход электричества за весь период','column_titles'=>array('Место',
	'Бригада', 'Удельный расход электричества, кВт*ч/т'), 'data'=>array(array(1,$table2[0][0],round($table2[1][0],2)),array(2,$table2[0][1],round($table2[1][1],2)),array(3,$table2[0][2],round($table2[1][2],2)),array(4,$table2[0][3],round($table2[1][3],2)))));
